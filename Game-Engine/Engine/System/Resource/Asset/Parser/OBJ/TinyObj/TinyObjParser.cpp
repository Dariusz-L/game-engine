...
#define IS_NEW_LINE(x) (((x) == '\r') || ((x) == '\n') || ((x) == '\0'))
	...
	static bool exportFaceGroupToShape(
		...
		const std::vector<tag_t> &tags, const std::string& materialName,
		...
		shape->mesh.materialName = materialName;
		shape->name = name;
	...
	void TrimNewLine(std::string& linebuf) {
	...
	void ParseVertex(const char* token, std::vector<float>& vertices) {
		token += 2;
		float x, y, z;
	...
	void ParseNormal(const char* token, std::vector<float>& normals) {
		token += 3;
		float x, y, z;
		parseFloat3(&x, &y, &z, &token);
		normals.push_back(x);
	...
	void ParseUV(const char* token, std::vector<float>& uvs) {
		token += 3;
		float x, y;
	...
	void ParseFace(const char* token, 
	...
	const char* CheckIfSkipLine(std::string& linebuf) {
		TrimNewLine(linebuf);
		...
		assert(token);
		if (token[0] == '\0') return nullptr;  // empty line
		if (token[0] == '#') return nullptr;  // comment line

		return token;
	}

	bool TryParseVertex(const char* token, std::vector<float>& vertices) {
		if (token[0] == 'v' && IS_SPACE((token[1]))) {
			ParseVertex(token, vertices);
			return true;
		}
	...
	bool TryParseNormal(const char* token, std::vector<float>& normals) {
		if (token[0] == 'v' && token[1] == 'n' && IS_SPACE((token[2]))) {
			ParseNormal(token, normals);
			return true;
		}
	...
	bool TryParseUV(const char* token, std::vector<float>& uvs) {
	...
	bool TryParseFace(const char* token, 
		std::vector<std::vector<vertex_index>>& faceGroup,
		std::vector<float>& v,
		std::vector<float>& vt,
		std::vector<float>& vn) {
		...
		if (token[0] == 'f' && IS_SPACE((token[1]))) {
			ParseFace(token, faceGroup, v, vt, vn);
			return true;
		}
	...
	bool LoadObj(attrib_t *attrib, 
		...
		// material
		std::string curUsedMaterialName("");
		...
		while (inStream->peek() != -1) {
			...
			const char* token = CheckIfSkipLine(linebuf);
			if (token == nullptr)									continue;

			if (TryParseVertex	(token, v))							continue;
			if (TryParseNormal	(token, vn))						continue;
			if (TryParseUV		(token, vt))						continue;
			if (TryParseFace	(token, faceGroup, v, vt, vn))		continue;
			...
			if ((0 == strncmp(token, "usemtl", 6)) && IS_SPACE((token[6]))) {
				...
				sscanf(token, "%s", namebuf);
#endif
				exportFaceGroupToShape(&shape, faceGroup, tags, namebuf, name, v, vt, vn);
				curUsedMaterialName = namebuf;
				faceGroup.clear();
			...
			if (token[0] == 'g' && IS_SPACE((token[1]))) {
				...
				bool ret = exportFaceGroupToShape(&shape, faceGroup, tags, curUsedMaterialName, name, v, vt, vn);
				if (ret) {
